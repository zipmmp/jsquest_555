...
const { ActionRowBuilder, ButtonBuilder, ButtonStyle, Collection, EmbedBuilder, StringSelectMenuBuilder } = require('discord.js');
...
class User extends EventEmitter {
    ...
    clearProcessListeners() {
        if (!this.process)
            return;
        if (this._onExit)
            this.process.off("exit", this._onExit);
        if (this._onMessage)
            this.process.off("message", this._onMessage);
    ...
    async sendCompleted() {
        ...
        if (!channel?.isTextBased())
            return;
        const user = client.users.cache.get(this.id) ?? await client.users.fetch(this.id).catch(() => null);
    ...
    async start() {
        if (!this.selectedQuest || !this.process)
            return false;
        const quest = this?.selectedQuest;
        if (!quest || this.started)
            return false;
    ...
    ConsoleString() {
        let lines = Array.from(new Set(this.logs.filter(d => d.trim().length > 0))).filter(d => d.trim().length > 0);
        lines = lines.map((line, i) => `[${i + 1}] ${line}`.trim());
        const maxLines = 15;
    ...
    genreate_message() {
        ...
        let rewards = quest.rewards.map(reward => {
        ...
        const buttonsRow = new ActionRowBuilder().setComponents(button).addComponents(refreshButton).addComponents(questLink);
        ...
        return { files, embeds: embeds, components: [new ActionRowBuilder().setComponents(menu), buttonsRow] };
    }
    destroy() {
