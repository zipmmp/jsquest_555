...
var RewardType;
(function (RewardType) {
    RewardType[RewardType["DiscordDecorations"] = 3] = "DiscordDecorations";
    RewardType[RewardType["DiscordOrb"] = 4] = "DiscordOrb";
    RewardType[RewardType["Nitro"] = 5] = "Nitro";
...
class Quest {
    ...
    formatTasks(i18n = this.i18n) {
        ...
        const tasks = this.progress.map((task) => {
            ...
            if (formattedName == i18nKey)
                formattedName = capitalizeWords(taskKey);
            // Format description
    ...
    get rewardLabel() {
        ...
        if (i18nKey == customRewardsLabel)
            customRewardsLabel = null;
        let text = reward.messages.name;
        if (customRewardsLabel)
            text += ` (${customRewardsLabel})`;
        return text;
    }
    isCompleted() {
    ...
    async getSolvedCount() {
        ...
        if (!quest) {
            quest = await questRepo.findOne({ where: { questId: this.id } });
            if (quest)
                await questsCache.set(this.id, quest);
        }
