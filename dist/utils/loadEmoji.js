...
export async function createEmojiFromUrl(client, emojiUrl, emojiName, roundImage = false, cropImage = false) {
    try {
        let uniqueEmojiName = emojiName;
        let emojiCache = client.application.emojis.cache.find((e) => e.name.trim().toLowerCase() === uniqueEmojiName.trim().toLowerCase());
        // If emoji already exists, generate a unique name
        if (emojiCache) {
            const timestamp = Date.now();
            uniqueEmojiName = `${emojiName}_${timestamp}`;
            emojiCache = client.application.emojis.cache.find((e) => e.name.trim().toLowerCase() === uniqueEmojiName.trim().toLowerCase());
        }
        if (emojiCache)
            return emojiCache.toString();
        // Fetch emoji image or video
        const response = await axios.get(emojiUrl, { responseType: "arraybuffer" });
        const isMp4 = emojiUrl.endsWith(".mp4") ||
...
export async function extractFirstFrame(videoBuffer, size = 128, name, roundImage = false, cropImage = false) {
